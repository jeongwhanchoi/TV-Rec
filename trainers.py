...
class Trainer:
    def __init__(self, model, train_dataloader, eval_dataloader, test_dataloader, args, logger):
        ...
        total_param = sum([p.nelement() for p in self.model.parameters()])
        if self.args.model_type in ['TVRec', 'Caser']:
            total_param -= self.model.position_embeddings.weight.numel()            
        self.logger.info(f"Total Parameters: {total_param}")

    def train(self, epoch):
        self.iteration(epoch, self.train_dataloader, train=True)

    def valid(self, epoch):
        if self.args.model_type == 'TVRec':
            for i in range(self.args.num_hidden_layers):
                self.model.item_encoder.blocks[i].layer.calculate_func_H()
    ...
    def load(self, file_name):
        ...
        new_dict = torch.load(file_name, map_location=torch.device('cuda:0'))
